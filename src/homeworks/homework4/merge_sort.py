...
def merge_sort_multithread_sublist(given_list: list[int], multiprocess: bool = False) -> list[int]:
    list_len = len(given_list)
    if list_len <= 1:
        return given_list
    pool = ProcessPoolExecutor if multiprocess else ThreadPoolExecutor
    with pool(max_workers=2) as executor:
        sublist1 = executor.submit(merge_sort, given_list[: list_len // 2])
        sublist2 = executor.submit(merge_sort, given_list[list_len // 2 :])
...
def merge_sort_multithread(unsorted_list: list[int], n_jobs: int, multiprocess: bool = False) -> list[int]:
    ...
    with pool(max_workers=n_jobs) as executor:
        sorted_parts = [
            executor.submit(merge_sort_multithread_sublist, single_slice, multiprocess) for single_slice in list_slices
        ]
