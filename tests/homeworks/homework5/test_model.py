...
class TestModel:
    ...
    @pytest.mark.parametrize(
        "mode, is_first_turn, ip, expected_opp",
        [
    ...
    def test_model_start_game(self, mode, is_first_turn, ip, expected_opp) -> None:
        self.model1.start_game(mode, is_first_turn, ip)
    ...
    @pytest.mark.parametrize(
        "mode, is_first_turn, ip, expected_opp",
        [("One computer", True, "", UserPlayer), ("One computer", False, "", UserPlayer)],
    )
    def test_model_start_game_one_computer(self, mode, is_first_turn, ip, expected_opp) -> None:
        self.model1.start_game(mode, is_first_turn, ip)

        assert isinstance(self.model1.players["player1"], UserPlayer)
        assert self.model1.players["player1"].side == "X" if is_first_turn else "0"
        assert self.model1.players["player1"].side == self.model1.players["player2"].side
        assert isinstance(self.model1.players["player2"], expected_opp)
